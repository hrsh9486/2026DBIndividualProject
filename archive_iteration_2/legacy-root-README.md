...
python3 scripts/promote_processed_data.py \
  ...
  capex-analysis/investment-quality.json \
  capex-analysis/state-capex-evaluation.json \
  capex-analysis/crowding-in-evidence.json
...
all chart contracts through the same validated CapEx build. The depth refresh
also lands RBI/MoSPI project-status and cost-overrun histories plus RBI state
capital-outlay and GSDP matrices, then builds an explicitly associational state
fixed-effects panel and a sample-gated crowding-in lag grid. It also publishes
