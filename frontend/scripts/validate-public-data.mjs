...
for (const [assetId, asset] of Object.entries(catalogue.assets ?? {})) {
  ...
  const detected = payload.series ? "dated_multi_series" : payload.markets ? "market_performance" : "unknown";
...
console.log(`Validated ${Object.keys(catalogue.assets).length} CapEx assets and ${catalogue.sections.length} section.`);
